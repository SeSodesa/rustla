...
impl TreeNode {
  ...
  pub fn child_is_allowed (&self, node_data: &TreeNodeType) -> bool {
    ...
    match self.data {
      ...
      TreeNodeType::Class { .. } => if node_data.node_categories().any(|cat| if let NodeCategory::Body = cat { true } else { false }) { true } else { false },
      ...
      TreeNodeType::Entry { .. } => if node_data.node_categories().any(|cat| if let NodeCategory::Body = cat { true } else { false }) { true } else { false },
