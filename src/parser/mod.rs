...
/// `std::option::Option::take` without invalidating the fields.
...
impl <'source> Parser <'source> {
    ...
    pub fn parse(&mut self) -> ParsingResult {
        ...
        // The parsing loop
        loop {
            // Check that we are not stuck parsing the same line over and over again
            ...
            // Flags for terminating the program if things don't seem to progress
            line_changed = false;
            let mut match_found = false;

            // Retrieve transition table of topmost state:
            ...
            for (pattern_name, regex, transition_function) in latest_state_transitions.iter() {
                ...
                // Run the current line of text through a DFA compiled from a regex
                if let Some(captures) = regex.captures(src_line) {
                    ...
                    self.doctree = match transition_function(
                        ...
                        // Transition succeeded ⇒ perform line advancements and state stack modifications
            ...
            // parsing in the previous state down the stack
            if ! match_found {
            ...
            // Line cursor pointing past source lines ⇒ terminate
