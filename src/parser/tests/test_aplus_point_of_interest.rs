...
.. point-of-interest:: Title text
  ...
  if let TreeNodeType::AplusPOI { title, id, previous, next, hidden, class, height, columns, bgimg, not_in_slides, not_in_book, no_poi_box, .. } = doctree.shared_child(0).shared_data() {
    ...
    assert_eq!(not_in_book.as_ref().unwrap(), r#"If this flag is given, this POI does not appear in the A+ content chapter."#);
    assert_eq!(no_poi_box.as_ref().unwrap(), r#"Removes surrounding box and navigation"#);
  } else {
    panic!()
  }
